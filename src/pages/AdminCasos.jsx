...
import { MessageCircle, Send, User, Building2, Shield, ArrowLeft, AlertTriangle, CheckCircle, Clock, Search } from 'lucide-react';
import BottomNav from '@/components/BottomNav';
import Header from '@/components/Header';
import { useNavigate, useParams } from 'react-router-dom';
import { getSupportCases, getSupportCaseById, addMessageToCase, updateSupportCase } from '@/lib/mockStore';
...
export default function AdminCasos() {
  ...
  const filtered = cases.filter(c => {
    ...
    if (search) {
      ...
      return c.vehicleLabel.toLowerCase().includes(q) || c.buyerName.toLowerCase().includes(q) || c.sellerName.toLowerCase().includes(q);
    }
    return true;
  });
            ...
            {filtered.map((c, i) => {
              ...
              return (
                <motion.div key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                  <Card
                    className="p-4 border border-border shadow-sm cursor-pointer hover:bg-card/80 transition-colors"
                    onClick={() => navigate(`/AdminCasos/${c.id}`)}
                  >
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">{c.messages.length} mensaje{c.messages.length !== 1 ? 's' : ''}</p>
                  </Card>
                </motion.div>
              );
            })}
...
export function AdminCasoDetalle() {
  ...
  const handleStatusChange = (newStatus) => {
    updateSupportCase(caseId, { status: newStatus });
    setCaseData(getSupportCaseById(caseId));
      ...
      <div className="px-4 py-3 border-b border-border bg-card">
        <div className="flex items-center justify-between mb-2">
          ...
          <Select value={caseData.status} onValueChange={handleStatusChange}>
            <SelectTrigger className="w-[140px] h-8 text-xs rounded-lg border-border">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        {caseData.messages.map((msg) => {
          ...
          return (
            <motion.div
              ...
              className={`flex gap-2 ${isMubis ? 'flex-row-reverse' : ''}`}
            >
              ...
              <div className={`max-w-[75%]`}>
                ...
                <div className={`rounded-2xl px-3.5 py-2.5 text-sm ${
                  isMubis
                    ? 'bg-accent/50 text-foreground border border-border rounded-tr-md'
                    : msg.senderRole === 'comprador'
                      ? 'bg-secondary/10 text-foreground rounded-tl-md'
      ...
      <div className="fixed bottom-20 left-0 right-0 px-4 pb-3 bg-muted border-t border-border pt-3 z-40">
        ...
        <div className="flex gap-2">
          <Input
            ...
            className="flex-1 h-11 rounded-full border-border bg-card text-sm"
          />
          <Button
            onClick={handleSend}
