...
function VehicleProcessCard({ v, navigate, inspection }) {
  const getStatusBadge = () => {
    ...
    // Solo mostrar "En peritaje" para vehículos en proceso
    if (['IN_PROGRESS', 'PENDING_INSPECTION', 'PENDING_PRICE_APPROVAL'].includes(v.status) || inspection && (inspection.status === 'IN_PROGRESS' || inspection.status === 'PENDING')) return <Badge className="bg-white text-secondary text-xs font-semibold border border-secondary/20">En peritaje</Badge>;
...
export default function MisSubastas() {
  ...
  const enProceso = useMemo(() => applyFilters(vehicles.filter((v) => ['PENDING_INSPECTION', 'IN_PROGRESS', 'PENDING_PRICE_APPROVAL'].includes(v.status))), [vehicles, search, filters]);
