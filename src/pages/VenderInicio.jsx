  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white pb-24">
      <div className="bg-gradient-to-b from-violet-700 to-violet-600 px-5 pt-8 pb-6 rounded-b-3xl">
        ...
        <h1 className="text-2xl font-bold text-white text-center mb-2">Mis Vehículos</h1>
        <p className="text-violet-200 text-center text-sm">Administra tus publicaciones</p>
      </div>
      ...
      <div className="px-4 py-5 -mt-4">
        ...
        >
          <Button 
            onClick={() => navigate(createPageUrl('PublicarCarro'))}
            className="w-full bg-violet-600 hover:bg-violet-700 h-14 rounded-xl font-bold text-lg shadow-lg mb-4"
          >
            <Plus className="w-6 h-6 mr-2" />
            Vender Nuevo Vehículo
          </Button>
        </motion.div>
                ...
                <Card className="overflow-hidden border-0 shadow-sm">
                  <div className="flex p-3 gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <p className="text-gray-500 text-xs">{vehicle.year} · {vehicle.plate}</p>
                        </div>
                        <Badge className={vehicle.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}>
                          {vehicle.status === 'active' ? 'Activa' : 'Borrador'}
                        </Badge>
                      </div>
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon"
                      onClick={() => navigate(`${createPageUrl('PublicarCarro')}?id=${vehicle.id}`)}
