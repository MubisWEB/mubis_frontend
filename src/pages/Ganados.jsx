...
import { Trophy, Phone, MapPin, CheckCircle, MessageCircle } from 'lucide-react';
import MubisLogo from '@/components/MubisLogo';
import BottomNav from '@/components/BottomNav';
import TopBar from "@/components/TopBar";

const demoWonAuctions = [
  { id: '1', brand: 'Mazda', model: '3', year: 2022, mileage: 15000, city: 'Bogotá', photos: ['https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/6931f0ed55d5d84622e39c62/8bac02287_IMG_3552.jpeg'], amount: 62000000, wonDate: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000), seller: { name: 'Carlos Mendez', phone: '+57 300 123 4567' }, status: 'pending_contact' },
  { id: '2', brand: 'Kia', model: 'Sportage', year: 2021, mileage: 28000, city: 'Medellín', photos: ['https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/6931f0ed55d5d84622e39c62/4185bae35_IMG_3560.jpeg'], amount: 78000000, wonDate: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000), seller: { name: 'María López', phone: '+57 310 987 6543' }, status: 'in_progress' },
  { id: '3', brand: 'BMW', model: 'X3', year: 2021, mileage: 22000, city: 'Cali', photos: ['https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/6931f0ed55d5d84622e39c62/faca62892_IMG_3590.jpeg'], amount: 145000000, wonDate: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000), seller: { name: 'Andrés García', phone: '+57 315 456 7890' }, status: 'completed' }
];
              ...
              <motion.div key={auction.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                <Card className="overflow-hidden border border-border shadow-sm rounded-2xl bg-card">
                  <div className="relative h-36">
                    ...
                    <div className="absolute top-3 right-3 bg-black/50 backdrop-blur-sm text-white text-xs px-2 py-1 rounded-full">{formatDate(auction.wonDate)}</div>
                  </div>
                  <div className="p-4">
                    ...
                    <div className="flex gap-2">
                      <Button className="flex-1 bg-secondary text-secondary-foreground hover:bg-secondary/90 rounded-full h-11"><Phone className="w-4 h-4 mr-2" />Llamar</Button>
                      <Button variant="outline" className="flex-1 border-secondary/20 text-secondary hover:bg-secondary/5 rounded-full h-11"><MessageCircle className="w-4 h-4 mr-2" />WhatsApp</Button>
                    </div>
                  </div>
                </Card>
              </motion.div>
