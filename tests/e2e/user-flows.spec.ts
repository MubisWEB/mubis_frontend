...
test.describe('business user flows', () => {
  test('dealer publishes a vehicle from the UI and sends it to inspection', async ({ page, request }) => {
    ...
    await selectDialogCombobox(page, 0, 'Mazda');
    await page.getByPlaceholder(/Ej: X3, Sportage/i).fill('3');
    await selectDialogCombobox(page, 1, '2023');
    ...
    await selectDialogCombobox(page, 2, 'Gasolina');
    await selectDialogCombobox(page, 3, /Manual/i);
    await selectDialogCombobox(page, 4, /Bogot/i);
